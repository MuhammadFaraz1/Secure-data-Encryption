...
def init_db():
    conection = sqlite3.connect("simple_data.db")
    c = conection.cursor()
    c.execute('''
...
st.title("Secure Data Encrytpion App")
...
if choice == "Store Secret":
    ...
    if st.button("Encrypt and Save"):
        if label and secret and passkey:
            ...
            try:
                c.execute("INSERT INTO vault(label,encrypted_text,passkey) VALUES (?,?,?)",(label,encrypted,hashed_key))
